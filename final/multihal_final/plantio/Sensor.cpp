...
std::vector<Event> PlantioSensor::readEvents() {
    std::ifstream in("/sys/kernel/plantio/sensors"); // open sysfs file to read sensor read
    std::vector<Event> events;
    ...
    if (in) {
        ...
        while(in >> sensorRead) {
            std::cout << "leitura do sensor multihal" << sensorRead << std::endl;
            measurements.push_back(sensorRead);
        }
        event_s.u.uncal.x = measurements[0];
        event_s.u.uncal.y = measurements[1];
        event_s.u.uncal.z = measurements[2];
        event_s.u.uncal.x_bias = measurements[3];
        event_s.u.uncal.y_bias = measurements[4];
